...
# unwritable/unreadable-as-expected at runtime — anything that tries to
# touch that directory (a lock file, an update check) can hang rather
# than fail fast, exactly what happened during first deployment (the app
# never became ready even after 120s, with well under half the allotted
# memory used — a stuck wait, not a slow-but-working load).
